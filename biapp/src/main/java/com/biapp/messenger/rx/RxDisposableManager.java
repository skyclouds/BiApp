...
public class RxDisposableManager {
    ...
    private static HashMap<Integer, CompositeDisposable> mDisposables = new HashMap<>();
    ...
    private void doAddDisposable(Object boundObject, Disposable disposable) {
        CompositeDisposable disposables = mDisposables.get(boundObject.hashCode());
        if (disposables == null) {
            disposables = new CompositeDisposable();
            disposables.add(disposable);
            mDisposables.put(boundObject.hashCode(), disposables);
        } else
            disposables.add(disposable);
    }
    ...
    private void doUnsubscribe(Object boundObject) {
        ...
        if (disposables != null) {
            disposables.dispose();
            mDisposables.remove(boundObject.hashCode());
        }
    }
}
